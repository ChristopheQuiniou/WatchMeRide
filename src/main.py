...
while True:
    ...
    if (bbox is not None):
        bbox = bbox[0]  # Extracting the coordinates from the nested array

        # Iterate through the four corners of the bounding box and draw lines
        for i in range(4):
            pt1 = (int(bbox[i][0]), int(bbox[i][1]))
            pt2 = (int(bbox[(i + 1) % 4][0]), int(bbox[(i + 1) % 4][1]))
